...
FROM dim_product AS p INNER JOIN fact_sales_monthly AS f
ON p.product_code=f.product_code
GROUP BY p.segment)
 
SELECT segment,unique_products_2020 AS product_count_2020,
        unique_products_2021 AS product_count_2021,
...
INNER JOIN fact_manufacturing_cost AS m ON p.product_code = m.product_code
WHERE m.manufacturing_cost = (SELECT MAX(manufacturing_cost) FROM fact_manufacturing_cost)
OR SELECT MIN(manufacturing_cost) FROM fact_manufacturing_cost)
ORDER BY m.manufacturing_cost DESC;

Alternative use IN & UNION:-  
...
INNER JOIN fact_pre_invoice_deductions AS pi
...
f.product_code,p.product AS product,
